...
const MapInterface = dynamic(() => import('@/components/MapInterface'), { ssr: false });
const TaggingModal = dynamic(() => import('@/components/TaggingModal'), { ssr: false });
const SensoryProfileModal = dynamic(() => import('@/components/SensoryProfileModal'), { ssr: false });
...
export default function Home() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!userData.sensoryProfile) {
          toast.info('Please complete your sensory profile');
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      if (!data.user.hasProfile) setShowProfileModal(true);
    } catch (error) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const tagMarkers = data.slice(0, 50).map(tag => ({
          ...
          icon: null,
          infoWindow: `<div style="padding:8px"><h3><strong>${tag.placeName || 'Location'}</strong></h3><p>Noise: ${tag.noiseLevel}/10</p><p>Light: ${tag.lightingLevel}/10</p><p>Crowd: ${tag.crowdDensity}/10</p></div>`
        }));
      ...
      navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        try {
          const res = await fetch('/api/emergency', {
            ...
            body: JSON.stringify({ latitude, longitude, triggerReason: 'User activated calm mode' })
          });
          if (res.ok) {
            const data = await res.json();
            ...
            const spaceMarkers = data.quietSpaces.map(space => ({
              ...
              lng: space.longitude,
              title: space.name,
              infoWindow: `<div style="padding:12px"><h3><strong>${space.name}</strong></h3><p>Type: ${space.type}</p><p>Distance: ${space.distance} km</p></div>`
            }));
            setMarkers(spaceMarkers);
            toast.success(`Found ${data.quietSpaces.length} quiet spaces!`);
          }
        } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-green-50">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {user ? (
                <>
                  <Button variant="outline" size="sm" onClick={() => setShowProfileModal(true)} className="hidden sm:flex">
                ...
                <>
                  <Button variant="outline" size="sm" onClick={() => { setIsLogin(true); setShowAuthModal(true); }}>
                    <LogIn className="w-4 h-4 mr-2" />
                    Login
                  </Button>
                  <Button size="sm" onClick={() => { setIsLogin(false); setShowAuthModal(true); }}>
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        {user && (
          <div className="mb-6">
            <Button onClick={handleEmergency} className="w-full bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white font-bold py-6 text-lg shadow-lg">
          ...
          <div className="lg:col-span-2">
            <Card className="p-4 bg-white/80 backdrop-blur-sm">
              <div className="mb-4 flex items-center justify-between">
                ...
                <Badge variant="secondary">{heatmapData.length} data points</Badge>
              </div>
              <MapInterface center={mapCenter} zoom={mapZoom} onMapClick={handleMapClick} markers={markers} heatmapData={heatmapData} />
          ...
          <div className="space-y-6">
            <Card className="p-6 bg-white/80 backdrop-blur-sm">
              ...
              <div className="space-y-3">
                ...
                <Button className="w-full justify-start" variant="outline" onClick={() => setShowProfileModal(true)} disabled={!user}>
                  <User className="w-4 h-4 mr-2" />
                  Update Sensory Profile
                </Button>
                <Button className="w-full justify-start" variant="outline" onClick={() => { loadTags(); loadHeatmapData(); toast.success('Map refreshed!'); }}>
              ...
              <Card className="p-6 bg-gradient-to-br from-blue-50 to-green-50 border-blue-200">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-white rounded-full h-2">
                        <div className="bg-orange-500 h-2 rounded-full" style={{ width: `${(sensoryProfile.noiseSensitivity / 10) * 100}%` }} />
                      </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-white rounded-full h-2">
                        <div className="bg-yellow-500 h-2 rounded-full" style={{ width: `${(sensoryProfile.lightSensitivity / 10) * 100}%` }} />
                      </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-white rounded-full h-2">
                        <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${(sensoryProfile.crowdTolerance / 10) * 100}%` }} />
                      </div>
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              {!isLogin && (
                <div>
                  ...
                  <Input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" required={!isLogin} />
                </div>
              )}
              <div>
              ...
              <div className="flex gap-3">
                <Button type="button" variant="outline" className="flex-1" onClick={() => setShowAuthModal(false)}>Cancel</Button>
                <Button type="submit" className="flex-1">{isLogin ? 'Login' : 'Sign Up'}</Button>
              </div>
      ...
      <TaggingModal isOpen={showTaggingModal} onClose={() => setShowTaggingModal(false)} location={selectedLocation} onSubmit={handleSubmitTag} />
      <SensoryProfileModal isOpen={showProfileModal} onClose={() => setShowProfileModal(false)} currentProfile={sensoryProfile} onSave={handleSaveProfile} />
    </div>
  );
}
